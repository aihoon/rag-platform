...
from ...api.schemas.chat import ChatRequest, ChatResponse, SourceDocument
...
async def chat(request: Request, response: Response, payload: ChatRequest) -> ChatResponse:
    ...
    logger.info(
        "chat request|chat_id=%s|company_id=%s|machine_id=%s|machine_cat=%s|rag_type=%s|dashboard_id=%s|model_id=%s",
        chat_id,
        company_id,
    ...
    meta = {
        ...
        "machineId": payload.service.machine_id,
        "machineCat": payload.service.machine_cat,
